...
Options:
  -c, --check           Validate OS, dependencies, and Go toolchain without installing
  -d, --dry-run         Simulate entire installation process and preview actions [Safe]
  -i, --install         Run live installation (Target host only)
...
check_go_installed() {
    ...
    local go_raw_ver
    go_raw_ver="$(go version 2>&1 || true)"
    local current_ver
...
main() {
    ...
    if [[ "${action}" == "install" ]]; then
        if [[ "${dry_run}" == "true" ]]; then
            printf "\n%b=== DRY-RUN SIMULATION (No System Mutations) ===%b\n" "${C_BOLD}${C_CYAN}" "${C_RESET}"
        ...
        printf "\n"
        if [[ "${deps_ok}" == "true" && "${go_ok}" == "true" ]]; then
            log_success "Pre-flight checks passed! Host is ready for Fabric installation."
        else
