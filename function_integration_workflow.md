...
1.  **Create New Version**: Copy the latest version of the main script from the `scripts/` directory to a new, incremented version file within the same directory (e.g., `cp scripts/myscript-v1.26.sh scripts/myscript-v1.27.sh`).
2.  **Copy the Function**: Copy the finalized, tested function from your temporary script and paste it into the `--- Core Functions ---` section of the new `scripts/myscript-vX.XX.sh` file.
...
4.  **Update Production Script**: Copy the new versioned script from the `scripts/` directory to the root directory to make it the active version (e.g., `cp scripts/myscript-vX.XX.sh myscript.sh`).
...
6.  **Commit and Push**: Use `git add` to stage the new version file, the updated root script, and the `README.md`. Commit the changes with a clear, descriptive message and push to the remote repository.
